...
import { captureChildStderr } from './log-streamer.js'

// Static ICO path — generated by scripts/generate-ico.mjs
const ICO_PATH = join(dirname(fileURLToPath(import.meta.url)), 'assets', 'tray.ico')
...
function startWindowsTray({ getStatus } = {}) {
  ...
  const iconPath = ICO_PATH
  const statusPath = join(dir, 'status.txt')
  writeWindowsTrayStatus(getStatus)

  const child = captureChildStderr(spawn('powershell.exe', [
    '-NoProfile',
    '-ExecutionPolicy',
    'Bypass',
    '-STA',
