...
if (Test-Path $envFile) {
    ...
    $nodeNameEnv = (Get-Content $envFile |
        Select-String 'SPINNY_NODE_NAME=(.+)' |
        ForEach-Object { $_.Matches[0].Groups[1].Value } |
        Select-Object -First 1)
}
if (-not $dashToken) {
...
if (-not $nodeNameEnv) {
    # spinny-<hostname-slug> — lowercase, hyphens only, max 24 chars
    $slug = ($env:COMPUTERNAME -replace '[^a-zA-Z0-9]', '-').ToLower().Trim('-')
    if ($slug.Length -gt 20) { $slug = $slug.Substring(0, 20) }
    $nodeNameEnv = "spinny-$slug"
}
...
SPINNY_NODE_NAME=$nodeNameEnv
SPINNY_ALLOW_INSECURE_FILE_KEY=1
"@
[System.IO.File]::WriteAllText($envFile, $envContent, [System.Text.UTF8Encoding]::new($false))
ok ".env written (node name: $nodeNameEnv)"
